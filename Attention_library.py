...
from keras.layers import concatenate, BatchNormalization, Conv1D
...
def CBAM_2018(input_feature, ratio=8):
    """
...
def CA_2021(x, reduction=32, bn_trainable=False):
    """
...
def ELA_2024(x, kernel_size=7, groups=8):
    """
    ...
    [b,h,w,channel] = x.shape
    
    # Compute mean along spatial dimensions
    x_h = AveragePooling2D(pool_size=(1, w), strides=(1, 1), data_format='channels_last')(x)
    x_w = AveragePooling2D(pool_size=(h, 1), strides=(1, 1), data_format='channels_last')(x)

    # Define layer normalization and activation (sigmoid)
    sigmoid = Activation('sigmoid')

    # Process along the height dimension
    x_h = tf.squeeze(x_h, axis=2)
    x_h = Conv1D(filters=channel, kernel_size=kernel_size, padding="same", use_bias=False)(x_h)
    x_h = tfa.layers.GroupNormalization(groups)(x_h) # Apply Group Normalization
    x_h = sigmoid(x_h)             # Apply Sigmoid activation
    x_h = tf.expand_dims(x_h, axis=2)
    
    # Process along the width dimension
    x_w = tf.squeeze(x_w, axis=1)
    x_w = Conv1D(filters=channel, kernel_size=kernel_size, padding="same", use_bias=False)(x_w)
    x_w = tfa.layers.GroupNormalization(groups)(x_w) # Apply Group Normalization
    x_w = sigmoid(x_w)             # Apply Sigmoid activation
    x_w = tf.expand_dims(x_w, axis=2)

    return x * x_h * x_w


def CA_2021_GN(x, reduction=32, groups = 8):
    """
    Coordinate Attention (CA).
    ...
    mip = max(8, c // reduction)
    y = Conv2D(filters=mip, kernel_size=(1, 1), strides=(1, 1), padding='valid')(y)
    y = tfa.layers.GroupNormalization(groups)(y) # Apply Group Normalization
    y = coord_act(y)
    x_h, x_w = Lambda(tf.split, arguments={'axis': 1, 'num_or_size_splits': [h, w]})(y)
    x_w = K.permute_dimensions(x_w, [0, 2, 1, 3])
    a_h = Conv2D(filters=c, kernel_size=(1, 1), strides=(1, 1), padding='valid', activation="sigmoid")(x_h)
